...
- 10번에 1번 꼴로 알맹이를 하나 더 주는 기능을 마무리 해보자
- SoldState클래스를 만들 때와 비슷하게 하면 됩니다
- 이제 한 가지만 더 바꾸면 됩니다
- 10% 확률로 당첨 여부를 결정해서 뽑기 기계의 상태를 WinnerState로 전환하는 기능을 추가하는 거죠
- 이 기능은 고객이 손잡이를 돌릴 때 작동해야 하므로 HasQuarterState에 추가하겠습니다
- 상태패턴을 사용하면 새로운 기능 추가를 간단히 구현할 수 있다

## 무엇이든 물어보세요

- Q) WinnerState가 꼭 있어야 하나요? 그냥 SoldState에서 알맹이를 2개 내보내도록 하면 안되나요?
- A) 좋은 질문입니다
  - 코드를 보면 사실 WinnerState는 SoldState와 거의 비슷합니다
  - 따라서 SoldState에서 알맹이를 2개 내보내는 방식으로 해도 별 문제는 없습니다
  - 대신 하나의 상태 클래스에서 2가지 상태를 표현하게 된다는 단점이 있습니다
  - 당첨이 된 상태와 그렇지 않은 상태가 모두 SoldState에서 표현하니까요
  - 코드 중복은 어느 정도 줄일 수 있지만, 대신 상태 클래스가 조금 불분명해집니다
  - 그리고 9장에서 배웠던 단일 역할 원칙에 위배되죠
  - WinnerState에서 맡을 역할을 SoldState에 넣게 되면서 SoldState역할이 2가지나 되니까요
  - 특별 행사 기간이 끝나면 어떻게 해야 할까요?
  - 아니면 당첨 확률이 달라지면 어떻게 해야 할까요?
  - 이런 몇 가지 장단점을 고려해야 하기에, 디자인 단계에서 여러분이 상황에 따라 가장 적합하다고 생각하는 방법을 선택해야 합니다

## 정상성 점검하기

- 최종 납품하기 전에 마지막으로 체크해야 할 몇 가지 사항

#### SoldState와 WinnerState에는 중복되어 있는 코드가 꽤 많기에 웬만하면 고쳐야 합니다

- State를 추상 클래스로 만들고 몇 가지 기본 기능을 추가할 수도 있습니다

#### dispense() 메소드는 항상 호출됩니다

- 심지어 동전 없이 손잡이를 돌려도 호출되죠
- turnCrank()에서 불리언 값을 리턴하게 하거나 예외를 도입해서 쉽게 이 문제를 해결할 수 있습니다

#### 상태 전환 정보는 모두 상태 클래스에 있습니다

- 이로 인해 어떤 문제가 생길까요?
- 그 부분은 GumballMachine 클래스로 옮기는게 좋을까요?
- 그렇게 할 때 어떤 장단점이 있을까요?

#### GumballMachine 객체의 인스턴스를 여러 개 만들게 될까요?

- 만약 그렇다면 상태 인스턴스를 정적 인스턴스 변수로 만들어서 공유하는 편이 좋습니다
- 그러려면 GumballMachine과 상태 클래스를 각각 어떻게 고쳐야 할까요?

## 방구석 토크 : 전략패턴 vs 상태패턴

- 전략 패턴) 오랜만이야, 친구. 내가 1장에 나왔다는 얘기 들었어?
- 상태 패턴) 그럼. 벌써 여기저기 소문이 다 났어
- 전략 패턴) 얼마전에는 템플릿 메소드를 조금 도와줬지. 내가 있어야 8장을 끝낼 수 있다나 뭐라나.
  - 그나저나 그동안 어떻게 지냈어?
- 상태 패턴) 그냥 똑같지 뭐. 클래스들이 서로 다른 상황에서 서로 다른 행동을 보여주는 걸 도와주는 거.
- 전략 패턴) 뭐 확실한 건 아니지만, 네가 하는 일이랑 내가 하는 일이 똑같아 보이는데, 그냥 설명만 다른 거 같아.
  - 한번 생각해보라고. 내가 하는 일은 구성과 위임으로 객체가 다른 행동과 알고리즘을 보이도록 해 주는 거 잖아.
  - 그런 데 네가 하는 것도 결국 내가 하는 거랑 똑같잖아
- 상태 패턴) 우리가 하는 일어 서로 연관되어 있다는 건 나도 인정하지만, 내가 하는 일이랑 네가 하는 일은 목적이 완전히 다르잖아.
  - 내가 클라이언트에게 구성과 위임을 사용하게 하는 방법은 네가 하는 방법이랑 완전히 다르단 말야
- 전략 패턴) 아, 그러셔? 어떻게 잘 이해가 안 되는데?
- 상태 패턴) 너는 어떤 클래스의 인스턴스를 만들고 그 인스턴스에게 어떤 행동을 구현하는 전략 객체를 건네주잖아
  - 1장에서 꽥꽥거리는 행동을 건네줬던 것 처럼
  - 진짜 오리에게는 꽥꽥거리는 행동을, 고무 오리에게는 삑삑 소리내는 행동을 줬었잖아
- 전략 패턴) 그랫었지. 그거 정말 대단하지 않았어? 상속보다 훨씬 강력한 방법이라는걸 너도 잘 알거야 그치?
- 상태 패턴) 물론 그렇지. 그럼 이번엔 내가 하는 방식을 생각해 보자고. 완전히 다르다고.
  - Context객체를 생성할 때 초기 상태를 지정해 줘도 되지만 그렇게 해도 그 후로는 그 Context 객체가 알아서 자기 상태를 변경하지
- 전략 패턴) 에이, 실행 중에 행동을 변경하는 건 나도 할 수 있다고.
  - 구성을 사용하면 원래 그렇게 할 수 있잖아
- 상태 패턴) 물론 그렇긴 하지. 하지만 나는 몇 가지 상태를 가지고 작업을 한다고.
  - Context 객체는 미리 정해진 상태 전환 귶칙에 따라 알아서 자기 상태를 변경하거든.
  - 그러니까 원래 상황에 맞게 `상태를 바꾸는 걸 염두에 둔 디자인`이라고 할 수 있어
- 전략 패턴) 그 점은 인정할게. 나는 객체가 상태를 변경하는 것을 상려하지는 않으니까
  - 객체에서 어떤 전략을 사용하는지는 보통 내가 직접 결정하거든
- 상태 패턴) 그것 봐. 구조가 비슷하다는 건 맞는 말이지만 그 용도나 목적은 확실히 다르다니까
  - 어떤 사람은 너를 활용할 테고, 어떤 사람은 나를 활용할 거야
- 전략 패턴) 알았어. 나는 1장에 나왔고, 너는 겨우 10장이나 되어서야 등장했잖아. 여기 까지 읽는 사람이 몇 명이나 될까?

## 리필 기능 추가해보기

- State 인터페이스에 refill() 메소드를 추가하고 SoldOutState를 제외한 나머지에선 아무 일도 하지 않습니다
- SoldOutState에서는 refill()메소드가 NoQuarterState로 전환됩니다.

## 디자인 도구 상자 안에 들어가야 할 도구들

- 10장도 끝났군요. 이 정도 공부했으면 이제 웬만한 기술 면접에는 끄덕 없을 겁니다.
...
- 클래스는 확장에는 열려 있지만 변경에는 닫혀 있어야 한다(Open/Closed Principle, OCP)
- 추상화된 것에 의존하게 만들고 구상 클래스에 의존하지 않게 만든다
- 진짜 절친에게만 이야기해야 한다
- 먼저 연락하지 마세요. 저희가 연락 드리겠습니다
- 어던 클래스가 바뀌는 이유는 하나뿐이어야만 한다
- (10장에서 새로 배운 원칙은 없음)
...
- 상태 패턴을 사용하면 Context 클래스의 내부 상태가 바뀜에 따라 객체가 알아서 행동을 바꿀 수 있도록 할 수 있습니다
- 상태 전환은 State 클래스로 제어할 수도 있고, Context 클래스로 제어할 수도 있습니다
- 상태 패턴을 쓰면 디자인에 필요한 클래스의 개수가 늘어납니다
- State 클래스를 여러 Context 객체의 인스턴스에서 공유하도록 디자인할 수도 있습니다(static or global state(전역 상태))
