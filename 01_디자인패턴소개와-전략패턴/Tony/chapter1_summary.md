...
- 식당에서 오고간 이야기
  ...
  - 공통으로 아는 전문 용어
    - 더 쉽게 대화할 수 있다
    - 디자인 패턴을 아직 모르는 사람에게 배우고 싶은 생각이 들도록 자극을 줄 수 있다
    - 객체 수준에서의 생각이 아닌, 패턴 수준에서 생각할 수 있기에 아키텍처를 생각하는 수준도 끌어올려준다

- 사무실 옆자리에서 들려온 이야기
  - 옵저버 패턴
  - 서로가 디자인 패턴을 알고 있다면 의사소통이 간결해진다

## 패턴과 전문 용어

- 패턴으로 소통하면 훨씬 효율적인 의사소통을 할 수 있음
- 서로 알고 있는 패턴을 정말 막강하다
  - 그 패턴에 담겨 있는 모든 내용, 특성, 제약조건 등을 함께 이야기할 수 있다
- 패턴을 사용하면 간단한 단어로 많은 얘기를 할 수 있다
- 패턴 수준에서 이야기하면 '디자인'에 더 오랫동안 집중할 수 있다
  - 구현과 관련된 자질구레한 내용 때문에 논점이 빗나가는 것을 최소화할 수 있다
- 전문용어를 사용하면 개발팀의 능력을 극대화할 수 있다
  - 디자인 패턴 용어를 모든 팀원이 잘 알고 있다면 오해의 소지가 줄어 작업을 빠르게 진행할 수 있다
- 전문 용어는 신입 개발자에게 훌륭한 자극제가 된다

## 디자인 패턴 사용법

- 라이브러리를 사용하면 방대한 분량의 코드를 그대로 활용할 수 있다
  - 하지만 라이브러리나 프레임워크가 더 좋은 구조를 만드는 데 도움을 주진는 못 한다
- 디자인 패턴은 라이브러리나 프레임워크가 도와주지 못 하는 것(더 좋은 구조)을 도와준다
- 스파게티 코드를 수정할 때 디자인 패턴을 적용하여 코드를 개선할 수 있다
- 여러 가지 디자인 패턴 -> 개발자의 두뇌 -> 디자인 패턴을 적용해서 새로 만든 멋진 코드

## 디자인 패턴을 바라보는 시각

- 객체지향 시스템을 구축하는 일은 그리 간단하지 않다
  - 상당한 노력을 기울여야만 할 수 있는 일
- 객체 지향 시스템 구축 방법들을 모아서 디자인 패턴을 만든 것
- 디자인 패턴은 예술이다
  - 장점이 있으면, 단점도 있는 법
  - 하지만 오랜 시간 동안 고민해서 찾아낸 디자인 패턴을 잘 따른다면 훨씬 좋은 디자인을 만들 수 있을 것이다
- 관리가 용이한 객체 지향 시스템을 만드는 비결
  - 나중에 어떻게 바뀔 것인지 생각해 보는 것
...
- 객체지향 기초
  ...
  - 추상화
  - 캡슐화
  - 다형성 : 동일한 종류의 생물체가 하나 이상 존재하는 것 - e.g. 인간의 혈액형
  - 상속
