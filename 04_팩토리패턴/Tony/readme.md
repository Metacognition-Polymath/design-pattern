...
- `추상 팩토리`라고 부르는 새로운 형식의 팩토리를 도입해서 피자 종류에 맞는 `원재료군`을 생산하는 방법을 구축
- 추상 팩토리로 `제품군`을 생성하는 인터페이스를 제공할 수 있습니다
  - 팩토리 메소드가 모여있는 팩토리를 만들어서 제품군을 생성하는 인터페이스를 제공
...
- orderPizza 메소드는 createPizza 메소드를 호출

```ts
const pizza = new CheesePizza(ingredientFactory);
```

- 원재료 팩토리를 사용하므로 여기서부터는 달라짐
- createPizza 메소드가 호출되면 원재료 팩토리가 돌아가기 시작
...
- prepare() 메소드를 호출하면 팩토리에 원재료 주문이 들어감
  - 나) ts에선 초기값이 없으면 에러가 나서 abstract Pizza의 생성자에서 초기화를 다 해줌
    - 이것 때문에 약간 불필요한 자원이 더 들어온 느낌
...
- orderPizza 메소드는 프레임워크로써 동작
  - 피자를 만들고 피자를 준비하고 피자를 굽고 피자를 자르고 피자를 포장하는 일을 수행
  - 피자를 만드는 일은 팩토리에게 맡김

## 추상 팩토리 패턴의 정의

- 팩토리 패턴을 하나 더 배웠습니다 !
- 제품군을 만들 때 쓸 수 있는 패턴이죠

![](./images/AbstractFactoryPattern.jpeg)

- **추상 팩토리 패턴**(Abstract Factory Pattern)

  - 구상 클래스에 의존하지 않고도 서로 연관되거나 의존적인 객체로 이루어진 `제품군`을 생산하는 인터페이스를 제공합니다
  - 구상 클래스는 서브클래스에서 만듭니다

- 추상 팩토리 패턴을 사용하면 클라이언트에서 추상 인터페이스로 일련의 제품을 공급받을 수 있습니다

  - 실제로 어떤 제품이 생산되는지는 전혀 알 필요가 없습니다
  - 따라서 클라이언트와 팩토리에서 생산되는 제품을 분리할 수 있습니다
    - 여기에선 NYPizzaStore가 Client가 된다

- 추상 팩토리에 들어있는 각 메소드는 사실 팩토리 메소드 같네요
  - 메소드는 추상 메소드로 선언되어 있고 서브클래스에서 오버라이드해서 객체를 만들던데
  - 그냥 팩토리 메소드 아닌가요?
- 추상 팩토리 패턴에서 메소드가 팩토리 메소드로 구현되는 경우도 종종 있습니다
  - 추상 팩토리가 원래 일련의 제품을 만드는 데 쓰이는 인터페이스를 정의하려고 만들어진 것이니까요
  - 그 인터페이스에 있는 각 메소드는 구상 제품을 생산하는 일을 맡고, 추상 팩토리의 서브클래스를 만들어서 각 메소드의 구현을 제공합니다
  - 따라서 추상 팩토리 패턴에서 제품을 생산하는 메소드를 구현하는 데 있어서 팩토리 메소드를 사용하는 건 너무나 자연스러운 일이라고 할 수 있죠

## 패턴 집중 인터뷰 - 팩토리 메소드 패턴과 추상 팩토리 패턴

- 추상 팩토리 패턴과 팩토리 메소드 패턴은 차이점도 많지만, 공통점도 많다보니 종종 혼동할 수 있다

#### 팩토리 메소드 패턴

- 클래스를 써서 제품을 만든다
- 상속으로 객체를 만든다
  - 팩토리 메소드 패턴으로 객체를 생성할 때는 클래스를 확장하고 팩토리 메소드를 오버라이드해야한다
- 클라이언트 코드와 인스턴스를 만들어야 할 구상 클래스를 분리시켜야 할 때 사용
  - 어떤 구상 클래스가 필요할지 미리 알 수 없을 때에 유용합니다
- 서브클래스를 만들고 팩토리 메소드를 구현하기만 하면 됩니다

#### 추상 팩토리 패턴

- 객체를 써서 제품을 만든다
- 구성(composition)으로 만든다
  - 제품군을 추상 형식으로 제공
  - 제품이 생산되는 방법은 이 형식의 서브클래스에서 정의
  - 팩토리를 사용하고 싶으면 일단 인스턴스를 만든 다음 추상 형식을 써서 만든 코드에 전달
  - 따라서 팩토리 메소드 패턴을 쓸 때와 마찬가지로 클라이언트와 실제 구상 제품이 분리되느 것
- 연관된 제품을 하나로 묶을 수 있다는 장점도 있음
- 클라이언트에서 서로 연관된 일련의 제품을 만들어야 할 때,
  - 즉, 제품군을 만들어야 할 때 사용
...
- 클래스는 확장에는 열려 잇어야 하지만 변경에는 닫혀 있어야 한다(OCP)
- _추상화된 것에 의존하게 만들고 구상 클래스에 의존하지 않게 만든다_
...
- 팩토리 메소드 패턴은 상속을 활용합니다
  - 객체 생성을 서브 클래스에게 맡기죠
  - 서브클래스는 팩토리 메소드를 구현해서 객체를 생산합니다
- 추상 팩토리 패턴은 객체 구성(composition)을 활용합니다
  - 팩토리 인터페이스에서 선언한 메소드에서 객체 생선이 구현되죠
