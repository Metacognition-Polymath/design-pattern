...
- 점점 더 많은 디지안 패턴이 발견됨에 따라 디자인 패턴ㅇ르 찾거나 같은 그룹에 속하는 패턴 끼리 비교하기 좋게, 종류에 따라 분류할 필요성이 생겼습니다
...
- 언제 패턴을 적용할지를올바르게 결정하려면 상당한 경험과 지식이 축척되어야 합니다
- 일단 간단한 해결책만으로 붖고하다고 확신을 가지면 해결해야 할 문제와 제약조건을 종합적으로 고려해 봐야합니다
...
- 이 정도까지 했을 때 패턴을 써도 괜ㅊ낳겠다 싶으면 그 패턴을 적용하면 되죠
...
- 하지만 발생 가능성이 높은 실질적인 변경에 대비해서 패턴에 적용ㅇ해야지, 가능성이 높지 않은 가상적인 변경에 대비해서 패턴을 적용하는 일은 바람직하지 않습니다
- 패턴을 디자인 단계에서만 고려해야 하는 건 아닙니다. 리팩터링 할 때도 패턴 도입을 고려할 수 있습니다

### 리팩터링과 패턴

- 리팩터링이란 코드를 변경해서 코드 구조를 개선하는 과정을 뜼합니다
...
- 바람직한 닝ㄹ입니다
- 그 과정에서 패턴을 쓰는 연ㅅ브을 하면서 다양한 경험을 쌓을 수 있으니까요
...
- 새로운 계층을 추가해야 할 수도 있는데, 그러면 복잡해지는 문제와 더불어 효율이 떨어지는 문제가 생길 수도 있습ㄴ디ㅏ
